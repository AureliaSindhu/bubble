;========================================================================
; Main Program
; Reads 8 numbers from the user, sorts them using bubble sort,
...
; Prompts the user to enter 8 numbers between 0 and 100,
; validates the input, and stores them in the ARRAY.
;========================================================================
READ_NUMBERS
    ; -- Save Registers --
    ADD     R6, R6, #-7         ; Allocate space on stack
    STR     R7, R6, #6          ; Save R7 (for RET)
    STR     R5, R6, #5          ; Save used registers
    STR     R4, R6, #4
    STR     R3, R6, #3
    STR     R2, R6, #2
    STR     R1, R6, #1
...
RN_LOOP
    ; -- Check if 8 numbers have been read --
...
RN_DONE
    ; -- Restore Registers --
    LDR     R0, R6, #0          ; Restore registers in reverse order of saving
    LDR     R1, R6, #1
    LDR     R2, R6, #2
    LDR     R3, R6, #3
    LDR     R4, R6, #4
    LDR     R5, R6, #5
    LDR     R7, R6, #6
    ADD     R6, R6, #7          ; Deallocate stack space
    RET

;========================================================================
; SUBROUTINE: ASCII_TO_INT
; Converts a null-terminated string of digits to a binary integer.
; Parameters: R0 = address of the string
; Returns: R0 = the converted integer
;========================================================================
ASCII_TO_INT
    ; -- Save Registers --
    ADD     R6, R6, #-5
...
; Sorts the 8-element ARRAY in ascending order using the bubble sort
; algorithm.
;========================================================================
BUBBLE_SORT
    ; -- Save Registers --
...
;========================================================================
; SUBROUTINE: DISPLAY_SORTED
; Prints the 8 numbers in the sorted ARRAY, separated by spaces.
;========================================================================
DISPLAY_SORTED
    ; -- Save Registers --
    ADD     R6, R6, #-7
...
; Prints the number in R0 as a decimal value (0-100).
; Parameters: R0 = the number to print
;========================================================================
PRINT_DECIMAL
    ; -- Save Registers --
    ADD     R6, R6, #-6
    ...
    STR     R2, R6, #1
    STR     R1, R6, #0
    ADD     R1, R0, #0          ; Copy the number to R1 to preserve it
...
;========================================================================
; Data Section
; All memory allocations, constants, and strings are here.
;========================================================================
; --- Memory Storage ---
ARRAY           .BLKW   #8
